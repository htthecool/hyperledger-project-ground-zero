...
describe('Starting tests for veggie-exchange', function() {
  ...
  it('website title should be veggie-exchange', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('veggie-exchange');
    })
  });
  ...
  it('navbar-brand should be veggie-exchange',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('veggie-exchange');
    });
  });
